...
## are disabled or the K-orbit of <pt> has at most PG_CANON_MIN_ORBIT
...
BindGlobal( "PGCanonicalFormSetup", function( A, agMats, pt, fpt, info, induce )
    ...
    if not CANON_FORM or not IsBound( A.kernelIsTail ) or IsBound( info.power )
    then
        return fail;
    fi;
    ...
    if Characteristic( F )^e <= PG_CANON_MIN_ORBIT then return fail; fi;
